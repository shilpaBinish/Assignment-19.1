...
logit <- train(raw_timestamp_part_1 ~ .,data=training,method="glm",metric="ROC",preProcess=c("scale","center"),trControl=dfcontrol)
dput(colnames(df3))
logit_pred<-predict(logit,testing)
cm<-confusionMatrix(logit_pred,testing$raw_timestamp_part_1,positive = "M")
cm